...
public abstract class CyberReaplAction extends Action 
{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		...
		////처리전 조회
		/*
		 * 보험료 자동대출 납부 상환	신청금액이 현 보험료 자동대출납부 원리금 이내인지 체크, 신청금액이 더 큰 경우 처리 진행 불가
		 * 
		 */
		...
		String seladvcnt =  request.getParameter("seladvcnt")	== null ? "0" : request.getParameter("seladvcnt");	
			...
			if(process.equals("R")||process.equals("Y") ){
				...
				AplpaymentManager manager1 = AplpaymentManager.getInstance();

				DataSet payment   = Pmanager.Inpayment(inputp2, inputp3);				
				DataSet plcynoinfo 		= manager1.findPlcynoinfo(inputp1);
	
	
				
				int apl_amt 		= plcynoinfo.getInt("apl_amt"); ////APL대출액
				int ard_apl 		= plcynoinfo.getInt("ard_apl");////APL대출이자
							
				int realpay1 = apl_amt + ard_apl; 
				int realpay2 = payment.getInt("apln_amt");				
				anyframe.log.Logger.debug.println("APL대출액  ==========="+apl_amt);
				anyframe.log.Logger.debug.println("APL대출이자  ==========="+ard_apl);
				anyframe.log.Logger.debug.println("APL대출액 + APL대출이자 ==========="+realpay1);		
				anyframe.log.Logger.debug.println("신청금액 ==========="+realpay2);		
						
				if(realpay1 < realpay2){
				
					 message = "[처리전조회]신청금액이 자동대출납부원리금을 초과하였습니다.";
					msg = message;
					url = urlname;
		...
		{			
			...
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//// (0701875)2007년 10월 16일 안병훈 : 이중지급 방지를 위해 기승인된 건이거나 처리중인 건 걸러냄
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////				sp 로그를 위한 로직... 시작
		...
		else if(process.equals("R"))////지연건인경우 자동 PGM처리와 접수일때 승인처리
		{
			...
			////{	
				...
				//// [05-B-353] 2005-08-22 이위혁. 조회/분개 기능 관련 수정. 인수명공백제거
