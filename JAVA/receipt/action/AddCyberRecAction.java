...
////ksps2000
import gtone.changeminer.receipt.manager.AddPremiumsearchManager;
...
public abstract class AddCyberRecAction extends Action 
{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		...
		inputp2.put("deptcd", "INT"); ////부서코드
		inputp2.put("init", "S9024"); ////사원번호
		inputp2.put("policy", policy); ////증권번호
		inputp2.put("option", "I"); ////옵션
		inputp2.put("amount", "           "); ////약정액
		inputp2.put("indat", "        "); ////지점접수일
		inputp2.put("reqdat", "        "); ////청구시작일
		inputp2.put("recvdate", "        "); ////청구종료일
		...
		String seladvcnt =  request.getParameter("seladvcnt")	== null ? "0" : request.getParameter("seladvcnt");	
			...
			if(process.equals("R")||process.equals("Y") ){
				...
				AddPremiumsearchManager manager1 = AddPremiumsearchManager.getInstance();
				
				DataSet premiumsearch 	= manager1.findPremiumsearch(inputp1);	
				DataSet findAddprem 	= manager1.findAddprem(inputp2);	////1940 sp에서 값을 가져옴.. 
				
			
								
				int realpay1 = premiumsearch.getInt("modp") * Integer.parseInt(seladvcnt);
				int realpay2 = findAddprem.getInt("addprem");				
								
				anyframe.log.Logger.debug.println("실입금액			==========="+realpay1);
				anyframe.log.Logger.debug.println("추가납입보험료 ==========="+realpay2);				
								
								
				if(realpay1 > realpay2){
				
					 message = "[처리전조회]실입금액이 추가납입보험료의 최대납입금액을 초과합니다.";
					msg = message;
					url = urlname;
		...
		{			
			...
			try
			{
				...
				spinputp_.put("srvc_cd","8"); ////처리업무코드
	...
	////			approval.put("errmsg1", "DGAAAA있읍니다.");
				...
				if(errrtn.equals("0"))////KSPS2000실행(은행,HOST:80 처리성공)
				{
					...
					DataSet rtnsave4 = returns.findRtnsave1(input2);
						...
						if(selPayment.getText("srvc_cd").equals("8"))
						{
							////LP에게 메일 보내기					
							input10.put("v_mail_type", "03");
