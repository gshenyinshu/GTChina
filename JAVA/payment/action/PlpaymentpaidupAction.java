...
public  class PlpaymentpaidupAction extends CybercenterAction
{
		...
		throws Exception
	{
		try
		{
			...
			////현대출가능금액 - E
			input.put("cnvdate", Util.replaceStr(payment.getText("apln_dt").trim(), ".", "")); //// 접수일자
			anyframe.log.Logger.debug.println("접수일자 : [" + Util.replaceStr(payment.getText("apln_dt").trim(), ".", "") + "] 기준 약관대출 Action에서 기본항목 입력==========="+input);
			DataSet plpaymentApln 	= managers.findPlpayment(input);  ////m0161sp1 접수일자 기준 약관대출금 조회
			...
			if(process.equals("Y"))
			{
				...
				DataSet approval = managers.getApproval(input4, input2, input5);////결재,policy_apln_no,proc_emp_no
			...
			else if(process.equals("L"))
			{
				...
				int cnt = plpayment.getCount("rid_nam");////보종명
				for(int i=0;i<cnt;i++)
				{
				...
				{
					bankerr = "1";
					message = "신청금액이 현대출가능금액보다 많습니다.";				
				}
				////현대출가능금액>=신청금액 - E
			}
			*/

			DataSet  working2after = managers.Working2after(Util.replaceStr(payment.getText("apln_dt"), ".", "")); //// 접수일기준 +2영업일을 구해옴.
			request.setAttribute("working2after",working2after.getText("comp_after")); //// 접수일기준 +2영업일
			...
			//// 해당증번이 이미지, 콜센터, 캡실쪽에 진행중인지 여부를 가져옴.
