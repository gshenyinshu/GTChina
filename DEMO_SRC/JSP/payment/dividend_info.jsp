...
<!---->
<jsp:useBean id="dividendinfo" 	class="anyframe.data.DataSet" scope="request"/>
...
<!--
	...
	//goProcess('S','1030000211','20061106','00000511825','7709301790016','20','16708052461')
...
%>
	...
	function goProcess1(cd1, cd2, cd3, cd4, cd5, cd6, cd7,cd8)
	{
		...
		var act_amt = <%=dividendinfo.getText("act_amt",0).trim()%>-0;
		var auth_cd = "<%=payment.getText("auth_cd")%>";
...
<input type="hidden" name="returnurl"  value="/payment/dividend_info.do">
...
<%
												...
												bcashvl = dividendinfo.getText("bcashvl",0).trim();
												refnprm = dividendinfo.getText("refnprm",0).trim();
												val_pua = dividendinfo.getText("val_pua",0).trim();
												acu_div = dividendinfo.getText("acu_div",0).trim();
												acc_div = dividendinfo.getText("acc_div",0).trim();
												pdffund = dividendinfo.getText("pdffund",0).trim();
												acrdpdf = dividendinfo.getText("acrdpdf",0).trim();
												tot_amt = dividendinfo.getText("tot_amt",0).trim();
												pl_prin = dividendinfo.getText("pl_prin",0).trim();
												pl_int = dividendinfo.getText("pl_int",0).trim();
												apl_amt = dividendinfo.getText("apl_amt",0).trim();
												ard_apl = dividendinfo.getText("ard_apl",0).trim();
												red_amt = dividendinfo.getText("red_amt",0).trim();
												act_amt = dividendinfo.getText("act_amt",0).trim();
												...
												tot_divid = Integer.parseInt(dividendinfo.getText("acu_div").trim()) + Integer.parseInt(dividendinfo.getText("acc_div").trim());//배당총액(A)
%>																
													<tr class="adm15"> 
														<th>경과해약환급금</th>
																	...
																	<td background="/img/edit/subtitle_bg.gif"> 
																		<table height="31" class="subtitle">
																			<tr> 
                                        															...
                                        															<td width="74" class="subleft"><a href="dividend_search.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">배당금조회</a></td>
																				<td width="88" class="select">계약기본사항</td>																				
                                        															<td width="90"><a href="dividend_list.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">배당금발생내역</a></td>
																				<th width="6">|</th>
																				<td width="90"><a href="dividend_paidup.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">제지급금내역</a></td>
																			</tr>
																		</table></td>
...
%>																					
																	<td class="le"><div align="left"><%=lstpdat%>/<%=trans.removeZero(userinfo.getText("pdcnt").trim())%>회</div></td>
																	<th>기납입보험료</th>
...
%>																			
													...
													<tr> 
														<td><table width="100%" class="adm_table">
											                                ...
											                                <tr class="adm21"> 
											                                  <th>신청일자</th>
											                                  <td> <div align="left"><%=payment.getText("apln_dt").trim()%></div></td>
											                                  <th>배당총액</th>
											                                ...
											                                <tr class="adm21"> 
											                                  ...
											                                  <th>예금주</th>
											                                  <td><div align="left"><%=payment.getText("acct_nm").trim()%></div></td>
											                                </tr>
											                                <tr class="adm21"> 
											                                  <th>은행</th>
																	...
																	%>																	
																	<td><div align="left"><%=banknm1%></div></td>
											                                  <th>계좌번호 </th>
											                                  <td><div align="left"><%=payment.getText("acct_no").trim()%></div></td>
											                                </tr>
											                                <tr class="adm21"> 
											                                  <th>연락처</th>
<%
...
%>														                                  
											                                  <td><div align="left"><%=payor_phone%></div></td>
											                                  <th>E-Mail</th>
											                                  <td><div align="left"><%=payment.getText("payor_email").trim()%></div></td>
											                                </tr>
											                                <tr class="adm21"> 
											                                  <th>상태</th>
<%
...
%>																				                                  
											                                  <td><div align="left"><input type="text" name="rsltname"  value="<%=rsltname%>" size="14" maxlength="10"  style="background-color:white; border-width:0; border-color:black; border-style:solid;" readonly >
																	</div></td>
