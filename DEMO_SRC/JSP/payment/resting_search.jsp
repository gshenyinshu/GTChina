...
<jsp:useBean id="restingsearch" 	class="anyframe.data.DataSet" scope="request"/>
...
<jsp:useBean id="pyrna" class="java.lang.String" scope="request"/> <!-- 예금주 주민번호 -->
<jsp:useBean id="strAcctSsn" class="java.lang.String" scope="request"/>
...
<%
	String	cltno	 	 = request.getParameter("cltno")	== null ? "" : request.getParameter("cltno");
	int 		count 	 = 0;
	int 		hamt  = 0;
...
<!--
	...
	function bankconfc(cd1, cd2, cd3, cd4)
	{
		if ("<%=deathResult%>" == "1") {
			if(!confirm("생명보험 협회로부터 계약자 사망으로 등록된 계약 입니다. \n 지점 확인 후 지급 바랍니다. \n\n 처리하시겠습니까?"))  {
				return;
...
<input type="hidden" name="cltno" value="<%=cltno%>">
...
<input type="hidden" name="strAcctSsn"  value="<%=strAcctSsn%>">
...
<input type="hidden" name="returnurl"  value="/payment/resting_search.do">
<input type="hidden" name="procgb"  value="E1">
																...
																<tr>
																	<td background="/img/edit/subtitle_bg.gif">
																		<table height="31" class="subtitle">
																			<tr>
																				<td class="select">휴면보험금조회</td>
																				<td><a href="resting_paidup.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">제지급금내역</a></td>
																			</tr>
																		</table></td>
...
<%
																int cnt	= restingsearch.getCount("plyno");
																for(int k=0;k<cnt;k++)
																{
																	int hamt_i = 0;
																	String htemp  = "";
																	htemp = restingsearch.getText("hamt",k).trim();
																	if(htemp.length() == 0)
																	{
																		hamt_i = 0;
																	}
																	...
																	{
																		hamt_i = Integer.parseInt(restingsearch.getText("hamt",k).trim());
																	}
																	if( hamt_i >0 )
																	{
																		count = count + 1;
...
<%
																...
																else
																{
																	...
																	for ( int i=0; i < cnt; i++ )
																	{
																		...
																		if(Integer.parseInt(restingsearch.getText("hamt",i).trim()) <= 0)
																		{
																			continue;
																		}
																		else
																		{
																			//해지일
																			String refdate = restingsearch.getText("refdate",i).trim();
																			if(refdate.length() == 8)
																			{
																				int temp_year = Integer.parseInt( refdate.substring(0,4) );
																				temp_year = temp_year - 2;
																				//2006.03.24 유섭교 cyberadmin 기능개선 PSR
																				refdate = Integer.toString(temp_year) +"."+ refdate.substring(4,6) +"."+ refdate.substring(6,8);
																			}
																			else
																			{
																				refdate = "";
																			...
																			//계약일
																			String issdte = restingsearch.getText("issdte",i).trim();
																			if(issdte.length() == 8)
																			{
																				issdte = issdte.substring(0,4) +"."+ issdte.substring(4,6) +"."+ issdte.substring(6,8);
																			}
																			else
																			{
																				issdte = "";
...
%>
																<tr class="adm12">
																	<td class="ce"><%=restingsearch.getText("plyno",i).trim()%></td>
																	<td class="ce"><%=restingsearch.getText("planname",i).trim()%></td>
																	<td class="ce"><%=restingsearch.getText("ownna",i).trim()%></td>
<%
																			String ownid = restingsearch.getText("ownid",i).trim();
																			if(ownid.length() == 13)
																			{
																				ownid = ownid.substring(0,6) + "-" + ownid.substring(6);
																			}
%>
																	<td class="ce"><%=ownid%></td>
																	<td class="ce"><%=refdate%></td>
																	<td><div align="right"><%=Util.addComma(restingsearch.getText("hamt",i).trim())%></div></td>
																</tr>
<%
																				hamt = hamt + Integer.parseInt(restingsearch.getText("hamt",i).trim());
																			}//else
																		}//for
																	}//else
...
%>
																<tr class="adm12">
																	<th colspan="5" class="ce">합&nbsp;&nbsp;계</th>
																	<td><%=Util.addComma(Integer.toString(hamt))%></td>
																</tr>
<%
																}
%>
													...
													<tr>
														<td><table width="100%" class="adm_table">
																...
																<tr class="adm21">
																	<th>휴면보험금</th>
																	<td class="adm21"><%=Util.addComma(payment.getText("apln_amt").trim())%></td>
																	<th>예금주</th>
...
%>
																	...
																	<th><a href="javascript:bankconfc('<%=bankcd1%>','<%=payment.getText("acct_no").trim()%>','<%=payment.getText("payor_ssn").trim()%>','<%=payment.getText("acct_ssn").trim()%>');" onFocus="blur()"><u>예금주확인</u></a></th>
<%
																	String bankconfc = "N";
																	String wbbank = "";
...
<%
															...
															{
%>
														<a href="javascript:getResult('H','<%=payment.getText("policy_no")%>','<%=payment.getText("indate").trim()%>','<%=trans.fillZero(payment.getText("apln_amt"),11)%>','<%=payment.getText("payor_ssn")%>','<%=bankcd1%>','<%=payment.getText("acct_no").trim()%>','<%=payment.getText("sending_num")%>')" onFocus="blur()"><img src="/img/common/btn_sbun.gif" border="0"></a>
														<a href="javascript:goReturn('<%=payment.getText("policy_apln_no")%>');" onFocus="blur()"><img src="/img/common/btn_return.gif" hspace="10"></a>
<%
															}
...
<%
															...
															{
%>
														<img src="/img/common/btn_recognize.gif"  style="cursor: hand;" id="btn1" onClick="goProcess('I','<%=payment.getText("policy_no")%>','<%=payment.getText("indate").trim()%>','<%=trans.fillZero(payment.getText("apln_amt"),11)%>','<%=payment.getText("payor_ssn")%>','<%=bankcd1%>','<%=payment.getText("acct_no").trim()%>')"></td>
<%
															}
...
<%
															...
															{
%>
														<img src="/img/common/btn_money.gif"  style="cursor: hand;" id="btn2" onClick="javascript:goRecogn('<%=payment.getText("policy_apln_no")%>','E1','<%=payment.getText("policy_no")%>','<%=payment.getText("indate").trim()%>','<%=trans.fillZero(payment.getText("apln_amt"),11)%>','<%=payment.getText("payor_ssn")%>','<%=bankcd1%>','<%=payment.getText("acct_no").trim()%>')"></td>
<%
															}
														}
														else
