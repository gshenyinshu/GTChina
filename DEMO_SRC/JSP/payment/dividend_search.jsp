...
<!---->
<jsp:useBean id="dividendsearch" 	class="anyframe.data.DataSet" scope="request"/>
...
<%
	String 	cnvdate    = request.getParameter("cnvdate")	== null ? "" : request.getParameter("cnvdate");
...
<!--
	...
	function bankconfc(cd1, cd2, cd3)
	{
		...
		//form.action = "dividend_search.do";
		form.submit();
	}
...
%>
	...
	function goProcess1(cd1, cd2, cd3, cd4, cd5, cd6, cd7,cd8)
	{
		...
		var act_amt = <%=dividendsearch.getText("act_amt",0).trim()%>-0;
		var auth_cd = "<%=payment.getText("auth_cd")%>";
...
<input type="hidden" name="cnvdate" value="<%=cnvdate%>">
...
<input type="hidden" name="returnurl"  value="/payment/dividend_search.do">
...
<%
													...
													bcashvl = dividendsearch.getText("bcashvl",0).trim();
													refnprm = dividendsearch.getText("refnprm",0).trim();
													val_pua = dividendsearch.getText("val_pua",0).trim();
													acu_div = dividendsearch.getText("acu_div",0).trim();
													acc_div = dividendsearch.getText("acc_div",0).trim();
													pdffund = dividendsearch.getText("pdffund",0).trim();
													acrdpdf = dividendsearch.getText("acrdpdf",0).trim();
													tot_amt = dividendsearch.getText("tot_amt",0).trim();
													pl_prin = dividendsearch.getText("pl_prin",0).trim();
													pl_int = dividendsearch.getText("pl_int",0).trim();
													apl_amt = dividendsearch.getText("apl_amt",0).trim();
													ard_apl = dividendsearch.getText("ard_apl",0).trim();
													red_amt = dividendsearch.getText("red_amt",0).trim();
													act_amt = dividendsearch.getText("act_amt",0).trim();
																	...
																	<td background="/img/edit/subtitle_bg.gif"> 
																		<table height="31" class="subtitle">
																			<tr> 
																				...
																				<th>|</th>
																				<td><a href="dividend_list.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">배당금발생내역</a></td>
																				<th>|</th>
																				<td><a href="dividend_paidup.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">제지급금내역</a></td>
																			</tr>
																		</table></td>
...
%>																				
													...
													<tr> 
														<td><table width="100%" class="adm_table">
																...
																<tr class="adm12"> 
																	<th>총배당예치금</th>
																	<td><%=Util.addComma(dividendsearch.getText("acu_div").trim())%></td>
																	<th>약관대출원리금</th>
<%
																	int pl_amt = 0;//약관대출원리금
																	pl_amt = Integer.parseInt(dividendsearch.getText("pl_prin").trim()) + Integer.parseInt(dividendsearch.getText("pl_int").trim());
%>																	
																	<td><%=Util.addComma(Integer.toString(pl_amt))%></td>
																</tr>
																<tr class="adm12"> 
																	<th>예치금이자</th>
																	<td><%=Util.addComma(dividendsearch.getText("acc_div").trim())%></td>
																	
                                    												<th>APL원리금</th>
<%
																	int real_amt = 0;//APL원리금
																	real_amt = Integer.parseInt(dividendsearch.getText("apl_amt").trim()) + Integer.parseInt(dividendsearch.getText("ard_apl").trim());
%>                                    												
																	<td><%=Util.addComma(Integer.toString(real_amt))%></td>
																</tr>
<%
																	...
																	tot_divid = Integer.parseInt(dividendsearch.getText("acu_div").trim()) + Integer.parseInt(dividendsearch.getText("acc_div").trim());//배당총액(A)
																	rent_amt = pl_amt + real_amt;//기대출원리금(B)
																	pay_amt =  tot_divid - rent_amt;//실지급액(A-B)
%>																			
																<tr class="adm12"> 
																	<th>배당총액(A)</th>
														
																	<td><%=Util.addComma(Integer.toString(tot_divid))%></td>
																	
                                    												<th>기대출원리금(B)</th>
																	<td><%=Util.addComma(Integer.toString(rent_amt))%></td>
																</tr>
																<tr class="adm12"> 
																	
                                    												<th>실지급액(A-B)</th>
																	<td colspan="3"><%=Util.addComma(Integer.toString(pay_amt))%></td>
																</tr>
															</table></td>
													</tr>
													<tr> 
													...
													<tr> 														
										                              <td><img src="/img/common/bu_cir02.gif"> <b>배당금신청 
										                                접수내역</b></td>
													</tr>
													<tr> 
														<td><table width="100%" class="adm_table">
...
<%															
															...
															if((payment.getText("auth_cd").equals("30") || payment.getText("auth_cd").equals("31") || payment.getText("auth_cd").equals("32") || payment.getText("auth_cd").equals("33")) && payment.getText("rslt").trim().equals("A"))
															{
%>														
