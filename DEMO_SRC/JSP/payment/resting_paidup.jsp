...
<jsp:useBean id="restingpaidup" 	class="anyframe.data.DataSet" scope="request"/>
...
<jsp:useBean id="strAcctSsn" class="java.lang.String" scope="request"/> <!-- 예금주 주민번호 -->
...
<input type="hidden" name="returnurl"  value="/payment/resting_paidup.do">
...
<!-- 로그인 여부를 직접 체크한다. 끝-->								
							...
							<tr> 
								<td> <table width="100%">
										<tr> 											
							                        <td><img src="/img/common/bu_cir01.gif"> <b>휴면보험금지급</b> 
							                        </td>
											<td align="right">
<!-- 2006.12.27 유섭교  0602294 사이버 고객센터 화면 수정 -->
																...
																<tr> 																	
												                                  <td background="/img/edit/subtitle_bg.gif"> 
												                                    <table height="31" class="subtitle">
												                                      <tr> 
												                                        <td><a href="resting_search.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">휴면보험금조회</a></td>
...
<%
																int cnt  = restingpaidup.getCount("cardcd");
																String rtn = restingpaidup.getText("rtn",0);
																
																if(cnt == 0 || !rtn.equals("0"))
																{
%>																
																<tr align="center"> 
...
%>														
																<tr class="adm12"> 
<%
																		String snddate = restingpaidup.getText("snddate").trim();
																		if(snddate.length() > 7)
																		{
...
<%
																		String regdate = restingpaidup.getText("regdate").trim();
																		if(regdate.length() > 7)
																		{
...
<%
																		String pcsdate = restingpaidup.getText("pcsdate").trim();
																		if(pcsdate.length() > 7)
																		{
...
%>																	
																	...
																	<td class="ce"><%=restingpaidup.getText("cardna",i).trim()%></td>
																	<td><div align="right"><%=Util.addComma(trans.removeZero(restingpaidup.getText("payamt",i).trim()))%></div></td>
																	
																	<%	banknm2 = restingpaidup.getText("bankna").trim();
																		bankcd2 = restingpaidup.getText("bankcd").trim();
																	...
																	<td class="ce"><%=restingpaidup.getText("bnkacc",i).trim()%></td>
																	<td class="ce"><%=restingpaidup.getText("payna",i).trim()%></td>
																	<td class="ce"><%=restingpaidup.getText("prcinit",i).trim()%></td>
																</tr>
<%
																		}
																	}
%>																
													...
													<tr> 														
                            										<td><img src="/img/common/bu_cir02.gif"> <b>휴면보험금신청접수내역</b></td>
													</tr>
													<tr> 
														<td><table width="100%" class="adm_table">
										                                <col width=20%>
