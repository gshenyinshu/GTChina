...
<jsp:useBean id="plpayment_detail" 	class="anyframe.data.DataSet" scope="request"/>
...
<!--
	...
	function bankconfc(cd1, cd2, cd3)
	{
		...
		form.action = "pl_payment_search.do";
		form.submit();
	}
...
%>
		...
		if (act_amt > 10000000) {
			...
			return;
		} else if (act_amt > 2000000 && auth_cd != "33" && auth_cd != "32") {
			alert("지급 권한에 맞지 않는 금액입니다.");
			return;
		}
	...
	function goProcess1(cd1, cd2, cd3, cd4, cd5, cd6, cd7,cd8)
	{
		...
		var act_amt = <%=plpayment.getText("act_amt",0).trim()%>-0;
		var auth_cd = "<%=payment.getText("auth_cd")%>";
...
<body onload="<%if( process.equals("L") ){%>goTimeout('<%=payment.getText("rslt")%>')<%}%>;onInit();" >
<table width="980">
<tr>
...
<input type="hidden" name="returnurl"  value="/payment/pl_payment_search.do">
...
<!--a href="/receipt/policy_view.do?searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">
								<img src="/img/common/btn_list.gif" hspace="10" border="0">
...
<%
											...
											bcashvl = plpayment.getText("bcashvl",0).trim();
											refnprm = plpayment.getText("refnprm",0).trim();
											val_pua = plpayment.getText("val_pua",0).trim();
											acu_div = plpayment.getText("acu_div",0).trim();
											acc_div = plpayment.getText("acc_div",0).trim();
											pdffund = plpayment.getText("pdffund",0).trim();
											acrdpdf = plpayment.getText("acrdpdf",0).trim();
											tot_amt = plpayment.getText("tot_amt",0).trim();
											pl_prin = plpayment.getText("pl_prin",0).trim();
											pl_int = plpayment.getText("pl_int",0).trim();
											apl_amt = plpayment.getText("apl_amt",0).trim();
											ard_apl = plpayment.getText("ard_apl",0).trim();
											red_amt = plpayment.getText("red_amt",0).trim();
											act_amt = plpayment.getText("act_amt",0).trim();
...
%>
													...
													<tr class="adm15">
														...
														String cintsum_temp = plpayment_detail.getText("cintsum",0);
														int cintsum = cintsum_temp.equals("")? 0 : Integer.parseInt( cintsum_temp );
														...
														if ( cintsum > 0 ) {
															fontStart = "<font color='#FCC402'><b>";
															fontEnd   = "</b></font>";
															style     = ";background-color:#D1D1DA";
														}
														%>
															<%=fontStart%>총배당예치금<%=fontEnd%>
														</th>
														<td><input type="text" name="textfield" class="box03"  style="text-align:right<%=style%>" readonly value="<%=Util.addComma(acu_div)%>"></td>
													</tr>
													<tr class="adm15">
														<th>
															<%=fontStart%>예치금이자<%=fontEnd%>
														</th>
														<td><input type="text" name="textfield" class="box03"  style="text-align:right<%=style%>" readonly value="<%=Util.addComma(acc_div)%>"></td>
													</tr>
													<tr class="adm15">
														<th>PDF</th>
...
%>
													<tr>
														<td>&nbsp;&nbsp;접수일자 조회(<%=payment.getText("apln_dt").trim()%>)</td>
													</tr>
													<tr>
														<td><table width="100%" class="adm_table">
																<col width=20%><col width=30%><col width=20%><col width=30%>
<%
															...
															//				2006.03.21 유섭교 0600069 Cyberadmin 기능개선
															//현재 기대출 원리금이라고 가져오는 것은 m0161sp1에서 가져오는 것인데.
															//이것은 apl원리금 + 기대출원리금을 합친 금액이다.
															//따라서 순수하게 기대출원리금만 가져와서 보여줘야 함.
															loanamt_num = 0;

															cnt = plpaymentApln.getCount("rid_nam");//보종명
															for(int i=0;i<cnt;i++)
															{
																basloan = basloan + Integer.parseInt(plpaymentApln.getText("basloan",i).trim());
																lstloan   = lstloan   + Integer.parseInt(plpaymentApln.getText("lstloan",i).trim());
																//loanamt   = loanamt  + Integer.parseInt(plpaymentApln.getText("loanamt",i).trim());
																refprem   = refprem  + Integer.parseInt(plpaymentApln.getText("refprem",i).trim());
																loanval   = loanval  + Integer.parseInt(plpaymentApln.getText("loanval",i).trim());

																//기대출 원리금란에 잘못 표시되는 문제
																loanamt_num += ( Integer.parseInt( plpaymentApln.getText("pl_prin",i).trim() ) +
																					Integer.parseInt( plpaymentApln.getText("pl_int",i).trim() ) );

															}


															factor = plpaymentApln.getText("factor",0).trim();
%>
																...
																<tr class="adm12">
																	...
																	<td>
																		<%
																		    //2006.03.21 유섭교 0600069 Cyberadmin 기능개선
																			//Util.addComma(Integer.toString(loanamt))
																			out.println( Util.addComma(Integer.toString(loanamt_num)) );
																...
																<tr class="adm12">
																	<th>현대출가능액</th>
																	<td><font color="#543CFB"><b><%=Util.addComma(Integer.toString(loanval))%></b></font></td>
																	<th>대출이율</th>
																	<td>
																		<%
																			if(!factor.equals(""))
																				out.println( df.format(Float.parseFloat(factor)) );
																		%>&nbsp;%
																	</td>
																</tr>
															</table></td>
													...
													<tr>
														<td>&nbsp;&nbsp;처리일자 조회(<%=Utility.getToday("yyyy.MM.dd")%>)</td>
													</tr>
<%
    } // 변액대출일경우만 if문 수행
%>
...
<%
															...
															int intLoanval100 = 0;
															cnt = plpayment.getCount("rid_nam");//보종명

															for(int i=0;i<cnt;i++)
															{
																basloan = basloan + Integer.parseInt(plpayment.getText("basloan",i).trim());
																lstloan   = lstloan   + Integer.parseInt(plpayment.getText("lstloan",i).trim());
																//loanamt   = loanamt  + Integer.parseInt(plpayment.getText("loanamt",i).trim());
																refprem   = refprem  + Integer.parseInt(plpayment.getText("refprem",i).trim());
																// 변액대출일경우만 80에서 가져옴.
																if("51".equals(plancd) || "53".equals(plancd) || "54".equals(plancd) || "55".equals(plancd) || "56".equals(plancd)) {
																	loanval       = loanval        + Integer.parseInt(plpayment.getText("loanval80" ,i).trim());
																	intLoanval100 = intLoanval100  + Integer.parseInt(plpayment.getText("loanval100",i).trim());
																}else {
																	loanval   = loanval  + Integer.parseInt(plpayment.getText("loanval",i).trim());
																}

																//기대출 원리금란에 잘못 표시되는 문제
																loanamt_num += ( Integer.parseInt( plpayment.getText("pl_prin",i).trim() ) +
																					Integer.parseInt( plpayment.getText("pl_int",i).trim() ) );

															}

															String strLoanval100 = "";
															// 변액대출일경우만 80에서 가져옴.
															if("51".equals(plancd) || "53".equals(plancd) || "54".equals(plancd) || "55".equals(plancd) || "56".equals(plancd)) {
																strLoanval100 = "(" + Util.addComma(Integer.toString(intLoanval100)) + ")";
															}

															factor = plpayment.getText("factor",0).trim();
%>
																...
																<tr class="adm12">
																	<th>현대출가능액</th>
																	<td><font color="#543CFB"><b><%=Util.addComma(Integer.toString(loanval)) + strLoanval100%></b></font></td>
																	<th>대출이율</th>
																	<td>
																		<%
																			if(!factor.equals(""))
													...
													<tr>
														<td><table width="100%" class="adm_table">
																...
																<tr class="adm21">
																	<th>신청금액</th>
																	<td class="adm21"><font color="#543CFB"><b><%=Util.addComma(payment.getText("apln_amt"))%></b></font></td>
																	<th>계좌번호 </th>
...
%>
<!-- <%="[" + payment.getInt("apln_amt") + "]"%>, <%="[" + Integer.parseInt(payment.getText("apln_amt")) + "]"%>, <%="[" + amt1 + "]"%>, <%="[" + amt2 + "]"%>, <%="[" + amt3 + "]"%>, <%="[" + amt4 + "]"%>, <%="[" + plancd + "]"%>, <%="[" + working2after + "]"%>, <%="[" + Utility.getToday("yyyyMMdd") + "]"%>, <%="[" + strAplnDt + "]"%>, <%="[" + payment.getText("rslt") + "]"%>, <%="[" + payment.getText("auth_cd") + "]"%> -->
<%
													// 변액대출일경우 금액체크를 하지 않음.
