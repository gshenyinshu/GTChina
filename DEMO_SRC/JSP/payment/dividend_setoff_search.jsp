...
<jsp:useBean id="dividendoffsearch" 	class="anyframe.data.DataSet" scope="request"/>
...
%>
	...
	function goRecogn(cd,cd1, cd2, cd3, cd4, cd5, cd6, cd7)	
	{
		...
		//{			
			...
			{		
				...
				form.action = "dividend_setoff_search.do";
				form.submit();
	...
	function goProcess1(cd1, cd2, cd3, cd4, cd5, cd6, cd7,cd8)
	{
		...
		var act_amt = <%=dividendoffsearch.getText("act_amt",0).trim()%>-0;
		var auth_cd = "<%=payment.getText("auth_cd")%>";
...
<input type="hidden" name="returnurl"  value="/payment/dividend_setoff_search.do">
...
<%
											...
											bcashvl = dividendoffsearch.getText("bcashvl",0).trim();
											refnprm = dividendoffsearch.getText("refnprm",0).trim();
											val_pua = dividendoffsearch.getText("val_pua",0).trim();
											acu_div = dividendoffsearch.getText("acu_div",0).trim();
											acc_div = dividendoffsearch.getText("acc_div",0).trim();
											pdffund = dividendoffsearch.getText("pdffund",0).trim();
											acrdpdf = dividendoffsearch.getText("acrdpdf",0).trim();
											tot_amt = dividendoffsearch.getText("tot_amt",0).trim();
											pl_prin = dividendoffsearch.getText("pl_prin",0).trim();
											pl_int = dividendoffsearch.getText("pl_int",0).trim();
											apl_amt = dividendoffsearch.getText("apl_amt",0).trim();
											ard_apl = dividendoffsearch.getText("ard_apl",0).trim();
											red_amt = dividendoffsearch.getText("red_amt",0).trim();
											act_amt = dividendoffsearch.getText("act_amt",0).trim();
											String banknm1 = "";
											String bankcd1 = "";
											...
											<td><table width="100%">
													...
													<tr> 
														<td><table width="100%" class="adm_table">
																...
																<tr class="adm12"> 
																	<th>총배당예치금</th>
																	<td><div align="right"><%=Util.addComma(dividendoffsearch.getText("acu_div").trim())%></div></td>
																	<th>약관대출원리금</th>
<%
																	int pl_amt = 0;//약관대출원리금
																	pl_amt = Integer.parseInt(dividendoffsearch.getText("pl_prin").trim()) + Integer.parseInt(dividendoffsearch.getText("pl_int").trim());
%>																		
																	<td><div align="right"><%=Util.addComma(Integer.toString(pl_amt))%></div></td>
																</tr>
																<tr class="adm12"> 
																	<th>예치금이자</th>
																	<td><div align="right"><%=Util.addComma(dividendoffsearch.getText("acc_div").trim())%></div></td>
																	
                                  													<th>APL원리금</th>
<%
																	int real_amt = 0;//APL원리금
																	real_amt = Integer.parseInt(dividendoffsearch.getText("apl_amt").trim()) + Integer.parseInt(dividendoffsearch.getText("ard_apl").trim());
%>                                       													
																	<td><div align="right"><%=Util.addComma(Integer.toString(real_amt))%></div></td>
																</tr>
<%
																	...
																	tot_divid = Integer.parseInt(dividendoffsearch.getText("acu_div").trim()) + Integer.parseInt(dividendoffsearch.getText("acc_div").trim());//배당총액(A)
																	rent_amt = pl_amt + real_amt;//처리전 원리금(B)
																	//pay_amt =  tot_divid - rent_amt;//처리후 원리금(B-A)
																	pay_amt =  rent_amt - tot_divid;//처리후 원리금(B-A)
%>																	
																<tr class="adm12"> 
																	<th>배당총액(A)</th>
																...
																<tr class="adm12"> 
																	<th>처리후 원리금(B-A)</th>
																	<td colspan="3"><div align="right"><%=Util.addComma(Integer.toString(pay_amt))%></div></td>
																</tr>
															</table></td>
													</tr>
													<tr> 
