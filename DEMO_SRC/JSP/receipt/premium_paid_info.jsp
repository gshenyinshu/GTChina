...
<!-- 로그인 여부를 직접 체크한다. 끝-->								
			...
			<tr> 
				<td> <table width="100%">
						<tr> 											
                     				<td><img src="/img/common/bu_cir01.gif"> <b>보험료수납</b></td>
							<td align="right">
<!-- 2006.12.27 유섭교  0602294 사이버 고객센터 화면 수정 -->
																...
																<tr> 																													
												                                  <td background="/img/edit/subtitle_bg.gif"> 
												                                    <table height="31" class="subtitle">
												                                      <tr> 
												                                        <td><a href="premium_paid_search.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">보험료조회</a></td>
...
%>													                                    																	
																...
																<tr class="adm21"> 																	
												                                   <th><a href="javascript:bankconfc('<%=payment.getText("bank_cd")%>','<%=payment.getText("acct_no")%>','<%=payment.getText("payor_ssn")%>');" onFocus="blur()"><u>예금주확인</u></a></th>
<%												                                   
																	String wbbank = "";
