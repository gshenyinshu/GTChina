...
<jsp:useBean id="plrepaymentlist" 	class="anyframe.data.DataSet" scope="request"/>
...
<input type="hidden" name="returnurl"  value="pl_repayment_list.do">
...
<!-- 로그인 여부를 직접 체크한다. 끝-->								
							...
							<tr> 
								<td> <table width="100%">
										<tr> 											
								                      <td><img src="/img/common/bu_cir01.gif"> <b>약관대출상환지급</b> 
								                      </td>
											<td align="right">
...
<%
																int cnt = plrepaymentlist.getCount("seq");
																String rtn = plrepaymentlist.getText("rtn",0);
																if(cnt == 0  || rtn.equals("1"))
																{		
%>
																<tr align="center"> 
...
<%
																		String procdat = "";
																		procdat = plrepaymentlist.getText("procdat",i);
																		if(procdat.length() > 0)
																		{
																		...
																		String effdat = "";
																		effdat = plrepaymentlist.getText("effdat",i);
																		if(effdat.length() > 0)
																		{
...
%>																
																	...
																	<td class="ce"><%=plrepaymentlist.getText("seq",i).trim()%></td>
																	<td><%=Util.addComma(plrepaymentlist.getText("loanamt",i).trim())%></td>
																	<td><%=Util.addComma(plrepaymentlist.getText("eflnamt",i).trim())%></td>
																	<td><%=Util.addComma(plrepaymentlist.getText("efreamt",i).trim())%></td>
																	<td><%=Util.addComma(plrepaymentlist.getText("int",i).trim())%></td>
																	<td><%=Util.addComma(plrepaymentlist.getText("totamt",i).trim())%></td>
																</tr>
<%
																	}
																}
