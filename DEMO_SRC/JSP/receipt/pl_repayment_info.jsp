...
<jsp:useBean id="plrepaymentinfo" 	class="anyframe.data.DataSet" scope="request"/>
...
<input type="hidden" name="returnurl"  value="pl_repayment_info.do">
...
<%
												...
												bcashvl 		= plrepaymentinfo.getText("bcashvl",0).trim();
												refnprm 		= plrepaymentinfo.getText("refnprm",0).trim();
												val_pua 		= plrepaymentinfo.getText("val_pua",0).trim();
												acu_div 		= plrepaymentinfo.getText("acu_div",0).trim();
												acc_div 		= plrepaymentinfo.getText("acc_div",0).trim();
												pdffund 		= plrepaymentinfo.getText("pdffund",0).trim();
												acrdpdf 		= plrepaymentinfo.getText("acrdpdf",0).trim();
												tot_amt 		= plrepaymentinfo.getText("tot_amt",0).trim();
												pl_prin 		= plrepaymentinfo.getText("pl_prin",0).trim();
												pl_int 		= plrepaymentinfo.getText("pl_int",0).trim();
												apl_amt 		= plrepaymentinfo.getText("apl_amt",0).trim();
												ard_apl 		= plrepaymentinfo.getText("ard_apl",0).trim();
												red_amt 		= plrepaymentinfo.getText("red_amt",0).trim();
												act_amt 		= plrepaymentinfo.getText("act_amt",0).trim();
													String banknm1 = "";
													String bankcd1 = "";
													String banknm2 = "";
													String bankcd2 = "";
																...
																<tr> 																	
												                                  <td background="/img/edit/subtitle_bg.gif"> 
												                                    <table height="31" class="subtitle">
												                                      <tr> 
												                                        <td class="subleft"><a href="pl_repayment_search.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">약대조회</a></td>
...
%>																	
																...
																<tr class="adm11"> 
																	<th>선납보험료</th>
																	<td><div align="right"><%=Util.addComma(userinfo.getText("pdf").trim())%></div></td>
																	<th>실효선택</th>
<%
...
%>																			
													...
													<tr> 														
                            										<td><img src="/img/common/bu_cir02.gif"> <b>약대상환접수내역</b></td>
													</tr>
													<tr> 
														<td><table width="100%" class="adm_table">
																<col width=20%><col width=30%><col width=20%><col width=30%> 
