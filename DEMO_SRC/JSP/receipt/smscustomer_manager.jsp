<%
/* -----------------------------------------------------------------------------
 * Project Title	: 고객용 sms 메세지 목록
 * File Name		: smscustomer_manager.jsp
 * Description		: 고객용 sms 메세지 목록
 * Programmer		: @
 * First Write		: 2004.09.20
 * Last Update		: 2005.10.04	melanie	사이버센터 프로젝트
 * ---------------------------------------------------------------------------*/
%>
...
<jsp:useBean id="smscustomer" class="anyframe.data.DataSet" scope="request"/>
<%
	//세션에서 가져오기
...
<!--
	...
	function winreflash() {
		document.form.action = "smscustomer_manager.do";
		document.form.submit();
	}		
	
	function goUpdate(cd, seq_no) {
		var path = "";
		path = "/receipt/smsmsg_add.do?smsgubun="+cd+"&seq_no="+seq_no;
		
		window.open(path, 'SMS', 'left=400,top=150,Width=630,Height=450, toolbar=no, fullscreen=no, directories=no, status=no, scrollbars=yes, menubar=no, location=no, resizable=no');	
...
%>
        		<a href="/receipt/smslp_manager.do" onFocus="blur()"><img src="/img/common/btn_lp.gif"></a></td>
        		</tr>
        		</table>
		</td>
		</tr>
...
<%
	int cnt = smscustomer.getCount("srvc_name");
	if(cnt == 0) {
%>
			<tr> 
...
%>
			<tr class="adm01"> 
			<td><div align="left"><%=smscustomer.getText("srvc_name",i)%></div></td>
			<td><div align="left"><%=smscustomer.getText("send_msg",i)%></div></td>
			<td><div align="left"><%=smscustomer.getText("send_err",i)%></div></td>
			<td>
<%
			//2005-10-10. 05-C-138. 이위혁. 결재권한세분화
			//if( auth_cd.equals("20") || auth_cd.equals("30") ) {
			if( auth_cd.equals("20") || auth_cd.equals("30") || auth_cd.equals("31") || auth_cd.equals("32") || auth_cd.equals("33") ) {
%>
			<a href="javascript:goUpdate('U','<%=smscustomer.getText("seq_no",i)%>')" onFocus="blur()"><img src="/img/common/btn_replace.gif"></a>
<%
			}
%>
			</td>
