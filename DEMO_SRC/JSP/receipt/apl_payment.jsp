...
<jsp:useBean id="aplpayment" 	class="anyframe.data.DataSet" scope="request"/>
...
<input type="hidden" name="returnurl"  value="apl_payment.do">
																...
																<tr> 
																	<td background="/img/edit/subtitle_bg.gif"> 
																		<table width="227" height="31" class="subtitle">
													                                      <tr> 
													                                        <td class="select">APL내역</td>
													                                        <td><a href="apl_payment_info.do?policy_apln_no=<%=policy_apln_no%>&ssn=<%=ssn%>&policy=<%=policy%>&searchgubun=<%=searchgubun%>&current=<%=current%>&fromdate=<%=fromdate%>&todate=<%=todate%>&fromamt=<%=fromamt%>&toamt=<%=toamt%>&modeind=<%=modeind%>&srvc_ind=<%=srvc_ind%>&rslt=<%=rslt%>&search=<%=search%>&key=<%=key%>" onFocus="blur()">계약기본사항</a></td>
													                                      </tr>
													                                    </table></td>
...
%> 																										
                            										<td><img src="/img/common/bu_cir02.gif"> <b>APL대출내역</b></td>
													</tr>
													<tr> 
														<td><table width="100%" class="adm_table">
...
<%
																int cnt = aplpayment.getCount("seq");
																String rtn = aplpayment.getText("rtn",0);
...
<%
																		String procdat = "";
																		procdat = aplpayment.getText("procdat",i);
																		if(procdat.length() > 0)
																		{
																		...
																		String effdat = "";
																		effdat = aplpayment.getText("effdat",i);
																		if(effdat.length() > 0)
																		{
...
%>																
																	...
																	<td class="ce"><%=aplpayment.getText("seq",i).trim()%></td>
																	<td align="right"><%=Util.addComma(aplpayment.getText("loanamt",i).trim())%></td>
																	<td align="right"><%=Util.addComma(aplpayment.getText("eflnamt",i).trim())%></td>
																	<td align="right"><%=Util.addComma(aplpayment.getText("efreamt",i).trim())%></td>
																	<td align="right"><%=Util.addComma(aplpayment.getText("int",i).trim())%></td>
																	<td align="right"><%=Util.addComma(aplpayment.getText("totamt",i).trim())%></td>
																</tr>
<%
																	}
																}
%>																</table></td>
													...
													<tr> 														
                            										<td><img src="/img/common/bu_cir02.gif"> <b>APL접수내역(계약일:<%=issdte%>)</b></td>
													</tr>
													<tr> 
														<td><table width="100%" class="adm_table">
																<col width=20%><col width=30%><col width=20%><col width=30%> 
