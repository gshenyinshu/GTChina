/**
 * Title                : AutopaymentinfoAction.java
 * Description  : 자동이체변경
...
////2005.10.06. 05-C-139. 이위혁. 접수시에도 기신청건 체크하도록 수정.
...
import gtone.changeminer.receipt.form.AplpaymentinfoForm;
import gtone.changeminer.receipt.manager.AplpaymentinfoManager;
...
/**
 * @struts.action
 *      path="/apl_payment_info"
 *      name="aplpaymentinfoForm"
 *      scope="request"
 *      validate="false"
 *
 * @struts.action-forward
 *      name="apl_payment_info"
 *      path="/apl_payment_info.jsp"
*
 * @struts.action-forward
 *      name="alert"
 *      path="/alert.jsp"   
 */
public  class AplpaymentinfoAction extends CyberReaplAction
{
		...
		throws Exception
	{
		try
		{
			Logger.debug.println("################ AplpaymentinfoAction Start ####################");
			...
			AplpaymentinfoForm aplpaymentinfoForm = (AplpaymentinfoForm) form;
			FormToDataset.copyProperties(input, aplpaymentinfoForm);			
			...
			AplpaymentinfoManager manager = AplpaymentinfoManager.getInstance();
			...
			DataSet aplpaymentinfo 	= manager.findAplpaymentinfo(input);
			...
			{
				aplpaymentinfo 	= manager.findAplpaymentinfo(input);
				userinfo		= manager.findUserinfo(input1, input2);
			}			
																
			request.setAttribute("aplpaymentinfo", aplpaymentinfo); 
			...
			Logger.debug.println("################ AplpaymentinfoAction End ####################");

		    	if( !bankerr.equals("-1") )
		    	{
	           		String msg = message;
	        		String url = "/receipt/apl_payment_info.do?policy_apln_no="+policy_apln_no+"&ssn="+ssn+"&policy="+policy+"&bnkyn="+bnkyn;
				...
				return mapping.findForward("apl_payment_info");
		    	}
