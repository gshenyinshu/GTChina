...
public  class PlpaymentlistAction extends CybercenterAction
{
		...
		throws Exception
	{
		try
		{
			...
			////20060530 change1	
			anyframe.log.Logger.debug.println("Action policy==========="+policy);
			...
			anyframe.log.Logger.debug.println("Log : [" + Util.replaceStr(payment.getText("apln_dt").trim(), ".", "") + "] 기준 약관대출 Action에서 기본항목 입력==========="+input);
			DataSet plpaymentApln 	= managers.findPlpayment(input);  //
			anyframe.log.Logger.debug.println("Log : [" + Util.replaceStr(payment.getText("apln_dt").trim(), ".", "") + "] 기준 약관대출 Action에서 기본항목 output==========="+plpaymentApln);
